...
- `shamanic_locale.py` — localization bundle (`STRINGS[lang][key]`) covering all six supported languages. Public surface: `t(lang, key, **kwargs)` for string lookup with `.format()`, `detect_corpus_lang(export_dir)` which reads the `<html lang="...">` attribute from `accessible_text.html` (fallback: lingua over the first ~30 sentences from `sentences.csv`), and `LANGUAGE_NAMES` (`pl/en/ru/fi/is/it` → human names used in LLM prompts).
...
When adding a new shamanic step that prints to a text artefact: add a localized header key (`yourstep.header`) to all six entries in `STRINGS`, and use `t(lang, 'yourstep.header')` at write time — never hard-code Polish/English strings in the writer.

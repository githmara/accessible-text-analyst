...
- `shamanic_locale.py` — i18n loader for YAML dictionaries under `dictionaries/{lang}/*.yaml`. Public surface: `t(lang, key, **kwargs)` flattens nested YAML keys to dotted form (`prophecy.fallback.loc`) and applies `.format()`; bundles are loaded lazily via `@lru_cache(maxsize=8)` with per-key fallback to `en`. `detect_corpus_lang(export_dir)` reads the `<html lang="...">` attribute from `accessible_text.html` (fallback: lingua over the first ~30 sentences from `sentences.csv`). `LANGUAGE_NAMES` (`pl/en/ru/fi/is/it` → human names used in LLM prompts). PyYAML is a hard dependency from v1.1.

...
`generate_report.py` reads the saved `.ipynb`, walks `cells`, and emits `export_results/<project_dir>/analysis_report.html` (project dir resolved from `config.json`'s `source_file`, mirroring the notebook's `_slugify`). The `<html lang="...">` attribute, the `<title>` and the markdown narrative all follow `UI_LANG` (`get_ui_lang()` from `shamanic_locale`). For `ui_lang != "ru"`, the notebook's Russian markdown cells are replaced by content from `dictionaries/{ui_lang}/narrative.yaml`; the cell-to-key mapping is the `NARRATIVE_KEYS` tuple at the top of `generate_report.py`, indexed by the sequential position of markdown cells in the notebook. `cell_qa_rag` keeps its narrative key for future use but is skipped in the report itself. Missing key in the YAML → fallback to the notebook's source 1:1. Key behaviors:

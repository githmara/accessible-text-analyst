...
- `generate_md.py` — converts the generated `raport_analizy.html` to `raport_dla_notebooklm.md` for NotebookLM. Reads from and writes to the same `export_results/<project_dir>/`. Strips accessibility spans because NotebookLM does not consume them.
